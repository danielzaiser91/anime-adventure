  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
      <div className="bg-deep-night border border-spirit-blue rounded-lg p-6 w-96 max-h-screen overflow-auto">
        <h2 className="font-cinzel text-celestial-gold text-xl mb-4">{t('ui.pause.title', 'Paused')}</h2>

        <div className="space-y-3 mb-4">
          <div>
            <div className="text-sakura font-rajdhani text-sm mb-2">{t('ui.settings.textSpeed', 'Text Speed')}</div>
          ...
          <div>
            <div className="text-sakura font-rajdhani text-sm mb-2">{t('ui.settings.language', 'Language')}</div>
            <div className="flex gap-2">
          ...
          <div>
            <div className="text-sakura font-rajdhani text-sm mb-2">{t('ui.settings.save', 'Save')}</div>
            <div className="flex flex-col gap-2">
              {slots.map(({ slot, info }) => (
                ...
                >
                  {saved === slot
                    ? <span className="text-jade">✓ {t('ui.save.saved', 'Saved!')}</span>
                    : info
                      ? `Slot ${slot + 1}: ${info.sceneNameKey}`
                      : `Slot ${slot + 1}: ${t('ui.title.empty', 'Empty')}`
                  }
                </button>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            {t('ui.pause.returnToTitle', 'Return to Title')}
          </button>
        </div>
      </div>
      ...
      {showConfirmTitle && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-60">
          <div className="bg-deep-night border border-red-500 rounded-lg p-6 w-72">
            <p className="text-white font-noto text-sm mb-4">{t('ui.pause.confirmReturn', 'Return to title? Unsaved progress will be lost.')}</p>
            <div className="flex gap-3">
              <button onClick={() => { useGameStore.setState({ currentPhase: 'title' }); setPaused(false); }} className="flex-1 px-3 py-2 bg-red-600 text-white font-rajdhani rounded hover:bg-red-700">Yes</button>
              <button onClick={() => setShowConfirmTitle(false)} className="flex-1 px-3 py-2 bg-void border border-gray-600 text-gray-300 font-rajdhani rounded hover:border-spirit-blue">No</button>
            </div>
          </div>
        </div>
      )}
